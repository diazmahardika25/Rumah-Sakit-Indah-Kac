  ...
  return (
    <div className="h-full flex flex-col">
       <div className="mb-4 pb-2 border-b border-slate-100">
        ...
        <p className="text-xs text-slate-500">Gateway Data Klinis Patuh Standar FHIR & HIPAA.</p>
      </div>

      <form onSubmit={handleSearch} className="flex gap-2 mb-6">
        <input 
            type="text" 
            placeholder="Masukkan No. Rekam Medis (RM ID)"
      ...
      {record ? (
        <div className="flex-1 overflow-y-auto animate-fade-in">
            ...
            <div className="bg-red-50 border-l-4 border-red-500 p-3 mb-4">
                <div className="flex items-start gap-2">
                    ...
                    <div>
                        <h4 className="text-xs font-bold text-red-700 uppercase">HIPAA Audit Trail Active</h4>
                        <p className="text-[10px] text-red-600 mt-1">
                            Akses ID: {record.auditor} <br/>
                            Timestamp: {record.lastAccess} <br/>
                            Access Level: Clinical_Read_Only (RBAC)
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex justify-between items-center">
                    <span className="font-semibold text-slate-700">Data Klinis (FHIR Resources)</span>
                    <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] rounded font-mono">JSON: Valid</span>
                </div>
                <div className="p-4 space-y-4">
                    <div>
                        <span className="text-xs text-slate-400 uppercase font-bold">Diagnosa Utama (ICD-10)</span>
                        <div className="mt-2 space-y-2">
                            {record.diagnosis.map((d: any, idx: number) => (
                                <div key={idx} className="flex justify-between items-center p-2 bg-slate-50 rounded border border-slate-100">
                                    ...
                                    <span className="font-mono text-xs bg-slate-200 px-2 py-1 rounded text-slate-600">{d.code}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-slate-100">
                        <span className="text-xs text-slate-400 uppercase font-bold">Catatan Klinis Terakhir</span>
                        <p className="text-sm text-slate-600 mt-1 italic">
                            "Pasien mengeluhkan nyeri dada ringan. EKG normal. Disarankan kontrol diet rendah gula."
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-4 text-center">
                <button onClick={() => setRecord(null)} className="text-xs text-slate-500 hover:text-red-500 underline">
                    Tutup Akses & Hapus Sesi (Secure Logout)
                </button>
            </div>
        </div>
      ) : (
        <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
            ...
            <p className="text-sm">Menunggu Input ID Pasien...</p>
            <p className="text-xs mt-2 text-center max-w-xs">Akses dibatasi hanya untuk staf medis yang berwenang sesuai protokol keamanan RS.</p>
        </div>
      )}
    </div>
  );
